...
ACMD_FUNC(mobinfo)
{
	...
	for (uint16 k = 0; k < count; k++) {
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			int droprate = mob_getdroprate(&sd->bl, mob, mob->dropitem[i].rate, drop_modifier, id->type, 2);
...
static TIMER_FUNC(autoatpots_timer)
{
	...
	if (sd->sc.option & OPTION_AUTOPOTS) {
		...
		add_timer(gettick() + sd->autopots.delay_rate, autoatpots_timer, sd->bl.id, 0);
	}
	return 0;
}
...
void autoattack_motion(struct map_session_data* sd, int mob_id) {
	...
	if (status->hp < (status->max_hp / 2) && !pc_is50overweight(sd) && autoattack_wing_exist(sd)) {
		if (!sd->autoattack_need_rest) {
			...
			if (target_id) {
				...
				if (!battle_check_range(src, target, range)) {
					...
					if (target_id && mob_count > 1) {
						target = map_id2bl(target_id);
						if (battle_gettarget(target)) {
							if (battle_gettarget(target) != sd->bl.id) {
								target_id = 0;
								target = NULL;
								return;
							}
	...
	if (!target_id && !sd->sc.getSCE(SC_HIDING) && !pc_issit(sd) && !sd->autoattack_need_rest) {
		for (i = 0; i <= 14; i++) {
