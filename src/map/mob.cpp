...
int mob_getdroprate(struct block_list* src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, int type, int from, mob_data * md)

{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			//puppy boosttpye
			if (type == IT_ETC) {
				if (sd->sc.getSCE(SC_ITEMBOOST_ETC))
					drop_rate_bonus += sd->sc.getSCE(SC_ITEMBOOST_ETC)->val1;
			}
			if (type == IT_ARMOR || type == IT_WEAPON) {
				if (sd->sc.getSCE(SC_ITEMBOOST_EQUIPMENT))
					drop_rate_bonus += sd->sc.getSCE(SC_ITEMBOOST_EQUIPMENT)->val1;
			}
			if (type == IT_CARD) {
				if (sd->sc.getSCE(SC_ITEMBOOST_CARD))
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	{ // Item Drop
		...
		int drop_rate2, drop_rate, drop_modifier = 100;
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			drop_rate = mob_getdroprate(src, md->db, md->db->dropitem[i].rate, drop_modifier, it->type, 1, md);
			drop_rate2 = mob_getdroprate(src, md->db, md->db->dropitem[i].rate, drop_modifier, it->type, 2, md);
			...
			if( mvp_sd && md->db->dropitem[i].rate <= battle_config.rare_drop_announce ) {
				...
				sprintf(message, msg_txt(NULL, 541), mvp_sd->status.name, md->name, it->ename.c_str(), (float)drop_rate2 / 100);
